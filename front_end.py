...
class Home:
    def __init__(self, root):
        ...
        self.label_check_balance.pack(pady=10)
    ...
    # Def SingleSig Address
    def copy(self, event):
        if event.state == 4 and event.keysym == 'c':
            content = self.text_key_address.selection_get()
            root.clipboard_clear()
            root.clipboard_append(content)
            return "break"
        elif event.state == 4 and event.keysym == 'v':
            self.text_key_address.insert('end', root.selection_get(selection='CLIPBOARD'))
            return "break"
        else:
            return "break"
